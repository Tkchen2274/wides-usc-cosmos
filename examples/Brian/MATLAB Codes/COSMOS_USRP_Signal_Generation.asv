...
% 
% 
% Rui Wang: original version.                   May 18th, 2013
% Rui Wang: change parameters for USRP.       Nov. 24th, 2015
% Rui Wang: add comments for f1=0.            May 5th, 2016
% Rui Wang: modify the scrip to ensure more ifft points and better
%                 accuracy for the time-domain signal.        Nov. 8th, 2017
%                 
% Yuning Zhang: Add the conversion to the signal to designated 
%                         frequency.                                       Nov. 23rd, 2020
...
N = 500; %N = 80; % number of IFFT points
...
f2 = 20e6; fix2 = floor(f2/df);% high frequency, index
...
BW = 2*f2;
actual_SR = 200e6;
step = round(N / (actual_SR * T0));

disp(' ')
disp(['BW = ', num2str(BW/1e6), 'MHz'])
disp(['actual_SR = ', num2str(actual_SR/1e6), 'MS/s'])
...
sample_tmp = signal_bb(1:step:end);
...
f = -fs/2:df:(fs/2-df);
figure
plot(f/1e6,20*log10(abs(ifftshift(fft(m_t_fs)))));
xlabel('Frequency [MHz]');ylabel('Pwr [dBm]');
title('FFT of final waveform');
grid minor
...
writematrix(s, ['../Signal/cosmos_', num2str(-f2/1e6), 'MHz_to_', num2str(f2/1e6), 'MHz_SR_', ...
    num2str(fs/1e6), 'MS_', num2str(T0*1e6), 'us.txt'], 'Delimiter',' ' );
